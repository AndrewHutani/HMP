...
if __name__ == "__main__":
    ...
    if mpjpe_data_all is not None and mpjpe_physics_gt_all is not None and mpjpe_fusion_all is not None:
                ...
                for rate_idx, downsample_rate in enumerate(downsample_rates):
                    ...
                    for start_idx in range(0, max_start_idx + 1, stride):
                        ...
                        if downsample_rate >= 1.0 or np.isclose(downsample_rate, 1.0):
                            ...
                            src_start = start_idx
                            src_end = min(start_idx + src_span, batch['q'].shape[1])
                            ...
                            for k, v in batch.items():
                                ...
                                src_input = v[:, src_start:src_end, ...]  # [B, input_len, ...] chronological
                                ...
                                res_in = resample_with_history_boundary_anchored(src_input, input_len, float(downsample_rate), antialias=False)

                                # 3) Keep output at original sampling rate, aligned to the original boundary

                                output_indices = np.arange(src_end,
                                                        src_end + output_len)
                            ...
                            for k, v in batch.items():
                                ...
                                src_input = v[:, start_idx:start_idx + input_len, ...]  # [B, input_len, ...]
                                ...
                                original_last_input_idx = start_idx + input_len - 1
        ...
        np.savetxt(save_directory+"resampled_physmop_data_consistent_output_mean.csv", mpjpe_data_mean, delimiter=",", header=",".join(header))
        np.savetxt(save_directory+"resampled_physmop_physics_consistent_output_mean.csv", mpjpe_physics_mean, delimiter=",", header=",".join(header))
        np.savetxt(save_directory+"resampled_physmop_fusion_consistent_output_mean.csv", mpjpe_fusion_mean, delimiter=",", header=",".join(header))
        np.savetxt(save_directory+"resampled_physmop_data_consistent_output_std.csv", mpjpe_data_std, delimiter=",")
        np.savetxt(save_directory+"resampled_physmop_physics_consistent_output_std.csv", mpjpe_physics_std, delimiter=",")
        np.savetxt(save_directory+"resampled_physmop_fusion_consistent_output_std.csv", mpjpe_fusion_std, delimiter=",")
    ...
    export_mean_error_resample_rate_csv(
        ...
        save_directory+"resampled_physmop_data_consistent_output_mean_with_error.csv",
        resample_rates=downsample_rates,
        time_labels=["80ms", "400ms", "560ms", "1000ms"]
    )
    export_mean_error_resample_rate_csv(
        physics_mean, physics_confidence_interval,
        save_directory+"resampled_physmop_physics_consistent_output_mean_with_error.csv",
        resample_rates=downsample_rates,
        time_labels=["80ms", "400ms", "560ms", "1000ms"]
    )
    export_mean_error_resample_rate_csv(
        gcnext_mean, gcnext_confidence_interval,
        save_directory+"resampled_gcn_consistent_output_gcn_mean_with_error.csv",
        resample_rates=downsample_rates,
        time_labels=["80ms", "400ms", "560ms", "1000ms"]
    )
    ...
    for branch, results in branch_results.items():
        ...
        plt.ylim(y_limits)
        plt.savefig(figure_directory + branch_filenames[branch])
        plt.close()
