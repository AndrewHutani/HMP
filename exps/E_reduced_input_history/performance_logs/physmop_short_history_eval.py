...
from dataset.physmop.base_dataset_test import BaseDataset_test
import numpy as np
...
# ------------------------------------------------------------------------
# CHANGE THESE VALUES TO CHOOSE WHICH MODEL TO EVALUATE
INPUT_LENGTH = 12
MODEL_CHECKPOINT = "ckpt/PhysMoP/hist_length_12.pt"
OUTPUT_LOG_SUFFIX = "hist_length_12.txt"

# ------------------------------------------------------------------------
config.hist_length = INPUT_LENGTH
config.total_length = config.hist_length + config.pred_length
config.DATASET_FOLDERS_TEST = {
    'AMASS': f'data/data_processed/amass_test_{config.total_length}.pkl',
}
...
if __name__ == "__main__":
    realtime_model = RealtimePhysMop(MODEL_CHECKPOINT, device='cpu', input_length=INPUT_LENGTH)

    log_files = [f"exps/E_reduced_input_history/performance_logs/physmop_data_mpjpe_log_{OUTPUT_LOG_SUFFIX}", 
                 f"exps/E_reduced_input_history/performance_logs/physmop_physics_mpjpe_log_{OUTPUT_LOG_SUFFIX}", 
                 f"exps/E_reduced_input_history/performance_logs/physmop_fusion_mpjpe_log_{OUTPUT_LOG_SUFFIX}"]
